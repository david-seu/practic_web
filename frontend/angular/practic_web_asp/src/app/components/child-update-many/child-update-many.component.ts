...
export class ChildUpdateManyComponent {
    ...
    saveChild(id: number, name: string, age: number, powers: string, rank: number) {
        let ok = true;
        ...
        if (ok) {
            this.updatedChildren.push({ id, name, age, powers, rank })
        }
    }

    submitRequest() {
        this.parentService.getConfirmRequest(sessionStorage.getItem("userId")!).subscribe(() => {
            sessionStorage.setItem("updatedChildren", JSON.stringify(this.updatedChildren));
