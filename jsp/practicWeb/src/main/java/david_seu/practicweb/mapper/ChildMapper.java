...
public class ChildMapper {
    ...
    public static Child toEntity(ChildDto childDto) {
        return new Child(childDto.getId(), childDto.getName(), childDto.getParent());
    }

    public static ChildDto toDto(Child child) {
        return new ChildDto(child.getId(), child.getName(), child.getParent().getId(), child.getParent());
    }
}
